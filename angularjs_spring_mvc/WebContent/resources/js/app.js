...
app.config(function($routeProvider) {
			...
			.when("/grafico/media_pedido", {//rota
				controller : "lojaController",//chama esse controle
				templateUrl : "grafico/media_pedido.html" //direciona para essa pagina 
			})
...
// configuraçoes do controller de clientes -----------------------------------------------
app.controller('clienteController', function($scope, $http, $location, $routeParams) {
	...
	if ($routeParams.id != null && $routeParams.id != undefined
		...
		$http.get("cliente/buscarcliente/" + $routeParams.id).success(function(response) {
			...
			document.getElementById("imagemCliente").src = $scope.cliente.foto;//carrega a imagen quando for editar
			
			//------------------ carrega estados e cidades do cliente em edicao
			setTimeout(function () {
	...
	// Responsï¿½vel por salvar o cliente ou editar os dados
	...
	$scope.listarClientes = function(numeroPagina) {
		...
		$http.get("cliente/listar/" + numeroPagina).success(function(response) {//fazendo a requisiçao
			...
			$scope.data = response;//trazendo a pagina de registro
			
			//---------Inicio total de pagina ----------
	...
	$scope.proximo = function () {												//se o numero da pagina for menor que o totadapagina pode avançar
		if (new Number($scope.numeroPagina) < new Number($scope.totalPagina)) {// se nao faz nada
		 $scope.listarClientes(new Number($scope.numeroPagina + 1));
		} 
	};
	
	$scope.anterior = function () {					//se o numero da pagina for maior que o totadapagina pode retonar
		if (new Number($scope.numeroPagina) > 1) {//se nao faz nada
		  $scope.listarClientes(new Number($scope.numeroPagina - 1));
		}
	};
	...
	$scope.removerCliente = function(codCliente) {
		$http.delete("cliente/deletar/" + codCliente).success(function(response) {//removendo
			$scope.listarClientes($scope.numeroPagina);//retornando a lista de cliente
...
// faz a identificaï¿½ï¿½o da posiï¿½ï¿½o correta da cidade do registro para mostrar em ediï¿½ï¿½o
...
// carregar cidades quando ï¿½ navegador chrome usando jQuery
...
function visualizarImg() {
	 var preview = document.querySelectorAll('img').item(1);
	  var file    = document.querySelector('input[type=file]').files[0];//pegando a imagen que foi adicionada no file
	  ...
	  if (file) {//se a imagen for valida 
	    reader.readAsDataURL(file);	//fica guardada	    
	  } else {
	    preview.src = "";//se nao limpa o campo de imagens
	  }
